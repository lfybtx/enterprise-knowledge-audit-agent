...
| 用户 | 密码 | 角色 | 说明 |
| --- | --- | --- | --- |
| `admin` | `admin123456` | admin | 默认管理员，可管理用户、知识库、索引和系统诊断 |
| 普通用户 | 启动后注册或由 admin 创建 | user | 默认普通用户，需要被授予知识库成员权限 |

旧版 `alice`、`bob` 和 `local-demo` 演示身份已移除，不应再使用旧文档中的密码登录。生产环境请立即修改或禁用默认管理员密码。

前端提供知识库和成员管理入口，管理员可以创建普通用户并授予知识库角色。第十六阶段加入了知识库成员管理和文档级 ACL：owner 可以维护成员，editor 可以上传和检索，viewer 只能查看授权内容。
...
当前界面评测基线为 122 条用例，Recall@1 为 98.2%，引用准确率为 98.2%，平均延迟约 1.0 ms。界面操作和演示路径见：[界面功能说明](docs/ui-walkthrough.md)。
